...
# ── Générer clé encryption N8N si absente ────────────────────────────────────
if [ -z "$N8N_ENCRYPTION_KEY" ]; then
    export N8N_ENCRYPTION_KEY=$(openssl rand -hex 32)
    echo "▶ Génération N8N_ENCRYPTION_KEY (sera perdue au redéploiement)"
fi

echo "✓ Variables N8N: Host=$DB_HOST, DB=$DB_DATABASE, User=$DB_USERNAME"
