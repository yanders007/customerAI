...
if [ $? -eq 0 ]; then
    ...
    echo ""
    echo "   📧 Configuration Email Brevo:"
    echo "   MAIL_HOST=smtp-relay.brevo.com"
    echo "   MAIL_PORT=587"
    echo "   MAIL_USERNAME=votre_email@exemple.com"
    echo "   MAIL_PASSWORD=xsmtpsib-VOTRE_CLE_SMTP_BREVO"
    echo "   MAIL_ENCRYPTION=tls"
    echo "   MAIL_FROM_ADDRESS=votre_email@exemple.com"
    echo "   MAIL_FROM_NAME=\"Support IA\""
    echo ""
    echo "   🔐 Configuration N8N (optionnel mais recommandé):"
    echo "   N8N_ENCRYPTION_KEY=<générez avec: openssl rand -hex 32>"
    echo "   (Si vide, une clé aléatoire sera générée à chaque déploiement)"
    echo ""
    echo "   ℹ️  Les variables DB_HOST, DB_DATABASE, etc. sont déjà configurées"
    echo "   ℹ️  N8N utilisera automatiquement la même base PostgreSQL que Laravel"
    echo ""
